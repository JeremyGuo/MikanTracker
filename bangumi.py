...
from model import Bangumi, TV, Movie, Torrent, TorrentStatus, TorrentType
import config
...
from notification import sendNotificationFinish
...
async def _StartNewBangumi(bangumi_id: int):
    with Session() as session:
        ...
        if bangumi is not None:
            coroutines[bangumi_id] = asyncio.create_task(TrackBangumi(bangumi))
...
async def AddNewBangumi(name: str, season: str, rss: str, regex_rule_episode: str, episode_offset: int, need_super_resolution: bool, checked: bool):
    with Session() as session:
        ...
        event_loop_bangumi.call_soon_threadsafe(asyncio.create_task, _StartNewBangumi(new_bangumi.id))
...
async def SuperResolutionFinished(hash: str):
    pass
    ...
    with Session() as session:
        ...
        if torrent.torrent_type == TorrentType.TV:
            ...
            if tv.need_super_resolution:
                return JSONResponse(status_code=500, content={'message': 'Not implemented.'})
